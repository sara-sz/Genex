...
def _normalize_payloads(obj: Any) -> List[Dict[str, Any]]:
    ...
    Return a flat list of dict payloads.
    """
...
def build_clean_case_report_pdf(all_payloads: List[Dict[str, Any]], out_pdf: str = "genex_case_report_clean.pdf") -> str:
    ...
    for i, payload in enumerate(all_payloads):
        ...
        genex_tbl = Table(summary_rows, colWidths=[1.9*inch, 0.7*inch, 0.55*inch, 1.6*inch])
...
def build_clean_case_report_from_json_dir(json_dir: str, out_pdf: str = "genex_case_report_clean.pdf") -> str:
    ...
    # First: load any json file, whether it stores one payload dict or a full list of payloads
    ...
    # Fallback: if nothing found, use per-case csvs only would not be enough for clean rebuild
    if not payloads:
