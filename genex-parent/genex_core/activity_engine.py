...
def _make_fallback_activities(
    ...
    for i, m in enumerate(next_steps.get("milestones", [])[:activities_per_category], start=1):
        ...
        fallback_activities.append({
            ...
            "extended_reason": "",
            "group_note": (
                "If another child is nearby, invite them to join as a helper or take turns. "
                "Having a sibling or friend present can make this more motivating and social."
            ),
        })
...
12. For each activity, add a "group_note" field: 1–2 sentences explaining how to
    adapt or include the activity when another child (sibling or friend) is present.
    Keep it practical and positive — e.g. "If a sibling is around, invite them to
    take turns too — this adds a social element and keeps the energy positive."
...
""".strip()
    ...
    try:
        ...
        for idx, activity in enumerate(bank["activities"], start=1):
            ...
            activity["group_note"] = activity.get("group_note", "")
